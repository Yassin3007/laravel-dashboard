        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-6 col-xs-12 mb-1">
                    <h2 class="content-header-title">{{ __('dashboard.user.management') }}</h2>
                </div>
                <div class="content-header-right breadcrumbs-right breadcrumbs-top col-md-6 col-xs-12">
                    <div class="breadcrumb-wrapper col-xs-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">{{ __('dashboard.user.title') }}
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-xs-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">{{ __('dashboard.user.list') }}</h4>
                                ...
                                <div class="card-block card-dashboard">
                                    @can('create_user')
                                        <a href="{{ route('users.create') }}" class="btn btn-primary mb-1">
                                            <i class="icon-plus2"></i> {{ __('dashboard.user.add_new') }}
                                        </a>
                                    @endcan
                                </div>
...
{{--                <td>{{ $user->image }}</td>--}}
                                                <td>
                                                    ...
                                                    @can('delete_user')
                                                        <form action="{{ route('users.destroy', $user->id) }}" method="POST" style="display: inline-block;">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('{{ __('dashboard.user.delete_confirm') }}');">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="{{ 2 + count(Schema::getColumnListing('users')) }}" class="text-center">{{ __('dashboard.user.no_records') }}</td>
                                            </tr>
                                        @endforelse
                                        </tbody>
                                    </table>
